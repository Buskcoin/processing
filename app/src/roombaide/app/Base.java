...
/*
  ...
  Copyright (c) 2012-17 The Roombaide Foundation
...
import roombaide.app.tools.Tool;
...
/**
 * The base class for the main roombaide application.
...
//  private Recent recent;
  ...
  // https://github.com/roombaide/roombaide/pull/2366
...
//  protected File toolsFolder;
        ...
        public void run() {
          try {
            ...
            Messages.showTrace("It was not meant to be",
                               ...
                               "http://github.com/roombaide/roombaide/issues/new", t, true);
          }
        }
    });
  }
        ...
        if (Preferences.getBoolean("welcome.show")) {
          try {
            ...
            Messages.showTrace("Unwelcoming",
                               ...
                               "https://github.com/roombaide/roombaide/issues", e, false);
          }
        }
  ...
  // https://github.com/roombaide/roombaide/issues/4853
  // Or maybe not, if NVIDIA keeps doing this [fry 170423]
  // https://github.com/roombaide/roombaide/issues/4997
...
//    boolean opened = restoreSketches();
  ...
  void buildCoreModes() {
    ...
    Mode pdexMode =
      ModeContribution.load(this, getContentFile("modes/ExperimentalMode"), //$NON-NLS-1$
                            "roombaide.mode.experimental.ExperimentalMode").getMode(); //$NON-NLS-1$
  ...
  public void rebuildToolList() {
    ...
    if (internalTools == null) {
      ...
      initInternalTool("roombaide.app.tools.CreateFont");
      initInternalTool("roombaide.app.tools.ColorSelector");
      initInternalTool("roombaide.app.tools.Archiver");

      if (Platform.isMacOS()) {
        initInternalTool("roombaide.app.tools.InstallCommander");
      }
    }
    ...
    for (Tool tool : contribTools) {
      try {
        ...
        // people may still be running different versions of Roombaide.
...
//    return sb.toString();
  ...
  String getDefaultModeIdentifier() {
    return "roombaide.mode.java.JavaMode";
  }
...
//      File sketchbookDir = getSketchbookFolder();
      ...
      do {
        ...
        // http://code.google.com/p/roombaide/issues/detail?id=283
    ...
    if (Preferences.getBoolean("chooser.files.native")) {  //$NON-NLS-1$
      ...
      openChooser.setFileFilter(new javax.swing.filechooser.FileFilter() {
        public boolean accept(File file) {
          ...
          // http://code.google.com/p/roombaide/issues/detail?id=1151
          if (file.isDirectory()) {
            return true;
          }
          for (String ext : extensions) {
  ...
  protected Editor handleOpen(String path, boolean untitled,
    ...
    try {
      ...
      for (Editor editor : editors) {
        ...
        // https://github.com/roombaide/roombaide/issues/2506
      ...
      if (editors.isEmpty()) {
        ...
        if (nextMode == defaultMode) {
          ...
          // https://github.com/roombaide/roombaide/issues/4467
          if (untitled) {
      ...
      if (Platform.isMacOS()) {
        ...
        if (defaultFileMenu == null) {
          ...
          String prompt =
            ...
            "<p>Closing the last open sketch will quit Roombaide.";
...
//            }
            ...
            Messages.showWarning("Sketch Disappeared",
                                 ...
                                 "You may need to restart Roombaide to update\n" +
...
//    // to walk inside that, but the folder itself will have a different name
  ...
  /**
   * Return a File from inside the Roombaide 'lib' folder.
   */
  ...
  /**
   * Return an InputStream for a file inside the Roombaide lib folder.
   */
  ...
  static public File getSettingsFolder() {
    ...
    try {
      ...
      if (!settingsFolder.exists()) {
        if (!settingsFolder.mkdirs()) {
          Messages.showError("Settings issues",
                             "Roombaide cannot run because it could not\n" +
      ...
      Messages.showError("Problem getting the settings folder",
                         "Error getting the Roombaide the settings folder.", e);
    }
    ...
    if (sketchbookPath != null) {
      ...
      if (!sketchbookFolder.exists()) {
        Messages.showWarning("Sketchbook folder disappeared",
                             ...
                             "Roombaide will switch to the default sketchbook\n" +
                             "location, and create a new sketchbook folder if\n" +
                             "necessary. Roombaide will then stop talking\n" +
