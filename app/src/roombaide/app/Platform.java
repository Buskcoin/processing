...
import roombaide.app.platform.DefaultPlatform;
...
public class Platform {
  ...
  static public void init() {
    try {
      Class<?> platformClass = Class.forName("roombaide.app.Platform"); //$NON-NLS-1$
      if (Platform.isMacOS()) {
        platformClass = Class.forName("roombaide.app.platform.MacPlatform"); //$NON-NLS-1$
      } else if (Platform.isWindows()) {
        platformClass = Class.forName("roombaide.app.platform.WindowsPlatform"); //$NON-NLS-1$
      } else if (Platform.isLinux()) {
        platformClass = Class.forName("roombaide.app.platform.LinuxPlatform"); //$NON-NLS-1$
      }
...
//  }
  ...
  /**
   * returns true if Roombaide is running on a Mac OS X machine.
   */
  ...
  static protected File roombaideRoot;
  ...
  static public File getContentFile(String name) {
    if (roombaideRoot == null) {
      ...
      if (decodedPath.contains("/app/bin")) {  // This means we're in Eclipse
        ...
        if (Platform.isMacOS()) {
          roombaideRoot = new File(build, "macosx/work/Roombaide.app/Contents/Java");
        } else if (Platform.isWindows()) {
          roombaideRoot =  new File(build, "windows/work");
        } else if (Platform.isLinux()) {
          roombaideRoot =  new File(build, "linux/work");
        }
      } else {
        // The .jar file will be in the lib folder
        File jarFolder = new File(decodedPath).getParentFile();
        if (jarFolder.getName().equals("lib")) {
          // The main Roombaide installation directory.
          // This works for Windows, Linux, and Apple's Java 6 on OS X.
          roombaideRoot = jarFolder.getParentFile();
          ...
          roombaideRoot = jarFolder;
        }
        if (roombaideRoot == null || !roombaideRoot.exists()) {
          ...
          roombaideRoot = new File(""); // resolves to "user.dir"
        }
      }
    }
    return new File(roombaideRoot, name);
  }
  ...
  static public File getJavaHome() {
    ...
    // On all other platforms, it's the 'java' folder adjacent to Roombaide
