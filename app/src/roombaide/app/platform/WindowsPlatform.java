...
/*
  ...
  Copyright (c) 2012-2013 The Roombaide Foundation
...
package roombaide.app.platform;
...
import roombaide.app.platform.WindowsRegistry.REGISTRY_ROOT_KEY;
...
// HKEY_LOCAL_MACHINE\SOFTWARE\JavaSoft\Java Development Kit\CurrentVersion\1.6\JavaHome -> c:\jdk-1.6.0_05
  ...
  */
  ...
  /**
   * Make sure that .pde files are associated with roombaide.exe.
   */
  protected void checkAssociations() {
    try {
      if (Preferences.getBoolean("platform.auto_file_type_associations")) {
        // Check the key that should be set by a previous run of Roombaide
        String knownCommand =
...
*/
    ...
    if (WindowsRegistry.createKey(rootKey, "Software\\Classes", ".pde") &&
        ...
        // Now associate the 'open' command with the current roombaide.exe
  ...
  // looking for Documents and Settings/blah/Application Data/Roombaide
  public File getSettingsFolder() throws Exception {
    try {
      ...
      // https://github.com/roombaide/roombaide/issues/3838
      ...
      String msg = Util.containsNonASCII(path) ?
        "Please move Roombaide to a location with only\n" +
        "ASCII characters in the path and try again.\n" +
        "https://github.com/roombaide/roombaide/issues/3543" :
        "Could not find JNA support files, please reinstall Roombaide.";
  ...
  /** Get the Documents and Settings\name\My Documents\Roombaide folder. */
    ...
    if (clib == null) {
      try {
        ...
        Messages.showTrace("JNA Error",
                           ...
                           "http://github.com/roombaide/roombaide/issues/new", ule, true);

        /*
        // Might be a problem with file encoding, use a default directory
        // https://github.com/roombaide/roombaide/issues/3624
