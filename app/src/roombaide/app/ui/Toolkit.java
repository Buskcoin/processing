...
public class Toolkit {
  ...
  static public void setMenuMnemonics(JMenuItem... menu) {
    ...
    final String[] kdePreDefStrs = { "&file", "&new", "&open", "open&recent",
      ...
      ".+&handbook", "&whatsthis", "report&bug", "&aboutroombaide", "about&kde",
      "&beenden", "&suchen",   // de
  ...
  /**
   * Return an Image object from inside the Roombaide 'lib' folder.
  ...
  /**
   * Get an ImageIcon from the Roombaide 'lib' folder.
   * @since 3.0a6
   */
...
//      System.err.println("does not exist: " + file);
  ...
  /**
   * Give this Frame the Roombaide icon set. Ignored on OS X, because they
...
//  }
  ...
  */
  ...
  static public int zoom(int pixels) {
    ...
    // https://github.com/roombaide/roombaide/issues/4902
...
//          System.out.println("  good " + w);
  ...
  static public Font getMonoFont(int size, int style) {
    if (monoFont == null) {
      try {
        ...
        // https://github.com/roombaide/roombaide/issues/2886
        // https://github.com/roombaide/roombaide/issues/4944
  ...
  static public Font getSansFont(int size, int style) {
    if (sansFont == null) {
      try {
        sansFont = createFont("RoombaideSansPro-Regular.ttf", size);
        sansBoldFont = createFont("RoombaideSansPro-Semibold.ttf", size);
  ...
  static private Font createFont(String filename, int size) throws IOException, FontFormatException {
    ...
    if (!fontFile.exists()) {
      ...
      // Roombaide. If it's not using the local JRE, it may be because of this
      // launch4j bug: https://github.com/roombaide/roombaide/issues/3543
      if (Util.containsNonASCII(Platform.getJavaHome().getAbsolutePath())) {
        msg += "Trying moving Roombaide\n" +
          "to a location with only ASCII characters in the path.";
      } else {
        msg += "Please reinstall Roombaide.";
      }
