 ...
 * compatible with the rest of Roombaide and we don't want any confusion.
 * <p>
 * Added JAI ImageIO to support lots of other image file formats [131008].
 * Also copied the Roombaide TGA implementation.
 * <p>
...
//      break;
   ...
   * created by Roombaide, so this calls our own loadImageTGA().
    ...
    try {
      ...
      */
      ...
      if (image == null) {
        ...
        // Might be an incompatible TGA or TIFF created by Roombaide
  ...
  private void writeVideoOnlyVFR(File movieFile, File[] imgFiles, int width, int height, double fps, QuickTimeWriter.VideoFormat videoFormat, /*boolean passThrough,*/ String streaming) throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < imgFiles.length && !p.isCanceled(); i++) {
        ...
        p.setNote(Language.interpolate("movie_maker.progress.roombaide", f.getName()));
        p.setProgress(i);
