...
public interface IRobotInterface {
	 ...
	 *
	 * @param velocity
	 *            the average velocity of the wheels in mm/s. Range is -500 to
	 *            500
	 * @param radius
	 *            the turn radius in mm. Range is -2000 to 2000<br>
	 * @throws IOException
	 */
	void drive(int velocity, int radius) throws IOException;
	 ...
	 *
	 * @param leftVelocity
	 *            Left wheel velocity in mm/s. Range is -500 to 500
	 * @param rightVelocity
	 *            Right wheel velocity in mm/s. Range is -500 to 500
	 * @throws IOException
	 */
	void driveDirect(int leftVelocity, int rightVelocity) throws IOException;
	...
	/**
	 * Returns a non-zero value if a light bump is detected. The returned value
	 * is a bitmap of the light bumps detected:
	 * <ul>
	 * <li>Bit 0: Left
	 * <li>Bit 1: Front left
	 ...
	 *
	 * @param songNumber
	 *            the number of the song the iRobot is to play.
	 * @throws IOException
	 * @see #song(int, int[])
	 * @see #song(int, int[], int, int)
	 */
	void playSong(int songNumber) throws IOException;
	 ...
	 *
	 * @param sensorId
	 *            One of the SENSORS_* constants
	 * @throws IOException
	 */
	void readSensors(int sensorId) throws IOException;
	 ...
	 *
	 * @param songNumber
	 *            The song number associated with the specific song. Must be in
	 *            the range 0 through 15. If you send a second Song command,
	 *            using the same song number, the old song is overwritten.
	 * @param notesAndDurations
	 *            An int array of even length. Every even index represents a
	 ...
	 */
	void song(int songNumber, int[] notesAndDurations) throws IOException;
	 ...
	 * @param startIndex
	 *            an index into an int array specifying the first note
	 * @param length
	 ...
	 *      iRobot Create 2 Open Interface (OI)</a>
	 */
	void song(int songNumber, int[] notesAndDurations, int startIndex, int length) throws IOException;

	/**
	 * Set the tailLight on or off
	 * 
	 * @param on
	...
	/**
	 * Pause Java execution until Spot button is pressed. While waiting, blink
	 * the LED next to the desired button.
	 *
	 * @param beep
	 *            If true, plays a tune. @
	 * @throws IOException
	 */
	void waitButtonPressed(boolean beep) throws IOException;
