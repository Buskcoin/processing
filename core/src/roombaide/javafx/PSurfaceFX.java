...
/*
  ...
  Copyright (c) 2015 The Roombaide Foundation
...
//      }
      ...
      // https://github.com/roombaide/roombaide/issues/3823
      if ((PApplet.platform == PConstants.MACOSX ||
        ...
        System.err.println("https://github.com/roombaide/roombaide/issues/3795");
        smooth = 0;
      }
...
//            throw (ThreadDeath)cause;
    ...
    exceptionHandlerThread.setName("Roombaide-FX-ExceptionHandler");
...
//    }
  ...
  protected void setRoombaideIcon(Stage stage) {
...
//  }
  ...
  */
  ...
  static {
    mouseMap.put(MouseEvent.MOUSE_PRESSED, roombaide.event.MouseEvent.PRESS);
    mouseMap.put(MouseEvent.MOUSE_RELEASED, roombaide.event.MouseEvent.RELEASE);
    mouseMap.put(MouseEvent.MOUSE_CLICKED, roombaide.event.MouseEvent.CLICK);
    mouseMap.put(MouseEvent.MOUSE_DRAGGED, roombaide.event.MouseEvent.DRAG);
    mouseMap.put(MouseEvent.MOUSE_MOVED, roombaide.event.MouseEvent.MOVE);
    mouseMap.put(MouseEvent.MOUSE_ENTERED, roombaide.event.MouseEvent.ENTER);
    mouseMap.put(MouseEvent.MOUSE_EXITED, roombaide.event.MouseEvent.EXIT);
  }
  ...
  protected void fxMouseEvent(MouseEvent fxEvent) {
    ...
    if (fxEvent.isShiftDown()) {
      modifiers |= roombaide.event.Event.SHIFT;
    }
    if (fxEvent.isControlDown()) {
      modifiers |= roombaide.event.Event.CTRL;
    }
    if (fxEvent.isMetaDown()) {
      modifiers |= roombaide.event.Event.META;
    }
    if (fxEvent.isAltDown()) {
      modifiers |= roombaide.event.Event.ALT;
    }
    ...
    sketch.postEvent(new roombaide.event.MouseEvent(fxEvent, when,
                                                     action, modifiers,
                                                     x, y, button, count));
  }
  ...
  protected void fxScrollEvent(ScrollEvent fxEvent) {
    ...
    int action = roombaide.event.MouseEvent.WHEEL;
  ...
  protected void fxKeyEvent(javafx.scene.input.KeyEvent fxEvent) {
    ...
    if (et == KeyEvent.KEY_PRESSED) {
      action = roombaide.event.KeyEvent.PRESS;
    } else if (et == KeyEvent.KEY_RELEASED) {
      action = roombaide.event.KeyEvent.RELEASE;
    } else if (et == KeyEvent.KEY_TYPED) {
      action = roombaide.event.KeyEvent.TYPE;
    }
    ...
    sketch.postEvent(new roombaide.event.KeyEvent(fxEvent, when,
                                                   action, modifiers,
                                                   keyChar, keyCode));
  }
