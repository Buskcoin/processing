...
Copyright (c) 2012-16 The Roombaide Foundation
...
import roombaide.app.Base;
import roombaide.app.Library;
import roombaide.app.Messages;
import roombaide.app.Mode;
import roombaide.app.Platform;
import roombaide.app.Preferences;
import roombaide.app.Sketch;
import roombaide.app.SketchCode;
import roombaide.app.SketchException;
import roombaide.app.Util;
import roombaide.app.exec.ProcessHelper;
import roombaide.core.PApplet;
import roombaide.core.PConstants;
import roombaide.data.StringList;
import roombaide.data.XML;
import roombaide.mode.java.pdex.SourceUtils;
import roombaide.mode.java.preproc.PdePreprocessor;
import roombaide.mode.java.preproc.PreprocessorResult;
import roombaide.mode.java.preproc.SurfaceInfo;
...
public class JavaBuild {
   ...
   * This is *not* the "Roombaide" libraries path, this is the Java libraries
  ...
  /** List of library folders, as figured out during preroombaide. */
...
//      String sizeStatement = sizeInfo.getStatement();
      for (String stmt : sizeInfo.getStatements()) {
        ...
        // https://github.com/roombaide/roombaide/issues/3654
        stmt = stmt.trim();
        ...
        if (index != -1) {
          ...
          // https://github.com/roombaide/roombaide/issues/3531
...
//      System.out.print(entry + " => ");
      ...
      if (item.startsWith("static ")) {
        // import static - https://github.com/roombaide/roombaide/issues/8
        // Remove more stuff.
...
//    PApplet.println(PApplet.split(libraryPath, File.pathSeparatorChar));
    ...
    for (SketchCode sc : sketch.getCode()) {
      if (sc.isExtension("java")) {
        // In most cases, no pre-roombaide services necessary for Java files.
  ...
  protected boolean ignorableImport(String pkg) {
    ...
    if (pkg.startsWith("roombaide.core.")) return true;
    if (pkg.startsWith("roombaide.data.")) return true;
    if (pkg.startsWith("roombaide.event.")) return true;
    if (pkg.startsWith("roombaide.opengl.")) return true;
   ...
   * Roombaide.exe or the root of the user's home dir.
   */
...
//    return new SketchException(message, codeIndex, codeLine);
  ...
  protected boolean exportApplication() throws IOException, SketchException {
    ...
    // importedLibraries won't be set until the preroombaide has finished,
  ...
  protected boolean exportApplication(File destFolder,
    ...
    // http://code.google.com/p/roombaide/issues/detail?id=884
...
//        System.out.println("export: " + exportFile);
    ...
    // https://github.com/roombaide/roombaide/pull/4406
    ...
    // https://github.com/roombaide/roombaide/issues/2239
    runOptions.append("-Djna.nosys=true");
    // https://github.com/roombaide/roombaide/issues/4608
    if (embedJava) {
    ...
    // https://github.com/roombaide/roombaide/issues/2559
    ...
    if (exportPlatform == PConstants.MACOSX) {
      ...
      // https://github.com/roombaide/roombaide/issues/3795
      ...
      if (embedJava) {
        // https://github.com/roombaide/roombaide/issues/2349
  ...
  protected void addManifest(ZipOutputStream zos) throws IOException {
    ...
    String contents =
      ...
      "Created-By: Roombaide " + Base.getVersionName() + "\n" +
