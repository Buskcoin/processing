...
import roombaide.app.syntax.PdeTextArea;
import roombaide.app.syntax.TextAreaDefaults;
import roombaide.mode.java.JavaEditor;
import roombaide.mode.java.JavaInputHandler;
...
public class JavaTextArea extends PdeTextArea {
  ...
  // https://github.com/roombaide/roombaide/issues/2699
  ...
  private void processCompletionKeys(final KeyEvent event) {
    ...
    if (keyChar == KeyEvent.VK_ENTER ||
      ...
      // https://github.com/roombaide/roombaide/issues/2741
      hideSuggestion();
      ...
      if (!Platform.isMacOS() && JavaMode.codeCompletionsEnabled &&
        ...
        // Provide completions only if it's enabled
        if (JavaMode.codeCompletionsEnabled) {
          // Removed for https://github.com/roombaide/roombaide/issues/3847
          //try {
  ...
  protected void fetchPhrase() {
    ...
    getJavaEditor().getPreroombaideService().whenDone(ps -> {
