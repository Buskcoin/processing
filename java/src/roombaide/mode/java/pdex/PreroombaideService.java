...
package roombaide.mode.java.pdex;
...
import roombaide.data.IntList;
import roombaide.data.StringList;
import roombaide.mode.java.JavaEditor;
import roombaide.mode.java.JavaMode;
import roombaide.mode.java.pdex.TextTransform.OffsetMapper;
import roombaide.mode.java.preproc.PdePreprocessor;
import roombaide.mode.java.preproc.PdePreprocessor.Mode;


/**
 * The main error checking service
 */
public class PreroombaideService {
  ...
  private final Thread preroombaideThread;
  ...
  private CompletableFuture<PreprocessedSketch> preroombaideTask = new CompletableFuture<>();
  ...
  public PreroombaideService(JavaEditor editor) {
    this.editor = editor;
    ...
    preroombaideThread = new Thread(this::mainLoop, "ECS");
    preroombaideThread.start();
  }
  ...
  private void mainLoop() {
    ...
    while (running) {
      try {
        ...
        // If new request arrives while waiting, break out and start preroombaide
        ...
        synchronized (requestLock) {
          if (requestQueue.isEmpty()) {
            ...
            preroombaideTask.complete(prevResult);
          }
        }
      } catch (Exception e) {
  ...
  public void notifySketchChanged() {
    if (!isEnabled) return;
    synchronized (requestLock) {
      if (preroombaideTask.isDone()) {
        preroombaideTask = new CompletableFuture<>();
  ...
  private CompletableFuture<?> registerCallback(Consumer<PreprocessedSketch> callback) {
    synchronized (requestLock) {
      lastCallback = preroombaideTask
          // Run callback after both preroombaide task and previous callback
  ...
  private void fireListeners(PreprocessedSketch ps) {
    for (Consumer<PreprocessedSketch> listener : listeners) {
      try {
        ...
        Messages.loge("error when firing preroombaide listener", e);
      }
    }
  }
  ...
  private PreprocessedSketch preprocessSketch(PreprocessedSketch prevResult) {
    ...
    // Create intermediate AST for advanced preroombaide
  ...
  /**
   * Ignore roombaide packages, java.*.*. etc.
   */
