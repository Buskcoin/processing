...
header {
package roombaide.mode.java.preproc;
}
...
webcolor_literal
    :   w:WEBCOLOR_LITERAL 
    { if (! (roombaide.app.Preferences.getBoolean("preproc.web_colors") 
    		 && 
        	 w.getText().length() == 6)) {
...
builtInType
    ...
    |   "color"              // aliased to an int in PDE
        { roombaide.app.Preferences.getBoolean("preproc.color_datatype") }? 
    ;
...
// types or methods, which requires semantic analysis (eg roombaide of import
...
primaryExpression
    :   (consCastTypeSpec[false] LPAREN) => constructorCast   
            { roombaide.app.Preferences.getBoolean("preproc.enhanced_casting") }?
