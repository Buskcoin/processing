...
package roombaide.mode.java.runner;

import roombaide.app.*;
import roombaide.app.exec.StreamRedirectThread;
import roombaide.app.ui.Toolkit;
import roombaide.core.*;
import roombaide.data.StringList;
import roombaide.mode.java.JavaBuild;
...
//    this.sketch = sketch;
      ...
      if (!library.supportsArch(PApplet.platform, variant)) {
        ...
        if (Platform.isMacOS()) {
          ...
          throw new SketchException(library.getName() + " is only compatible " +
                                    "with the  " + opposite + "-bit download of Roombaide.");
   ...
   * https://github.com/roombaide/roombaide/issues/4738
   */
...
//    PApplet.printArray(sketchParams);
    ...
    // https://github.com/roombaide/roombaide/issues/4098
...
//      throw new Error("Unable to launch target VM: " + exc);
  ...
  protected StringList getSketchParams(boolean present, String[] args) {
    ...
    if (build.getFoundMain()) {
      ...
      params.append("roombaide.core.PApplet");
      ...
      if (editor != null) {  // if running roombaide-cmd, don't do placement
